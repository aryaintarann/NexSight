...
export async function runSecurityScan(
  ...
  const [sslResult] = await Promise.all([checkSslTls(url)])
  ...
  allIssues.push(...headerResult.issues, ...sslResult.issues, ...mixedResult.issues, ...cookieResult.issues, ...owaspResult.issues)

  // Weighted: SC-01(30%) SC-02(25%) SC-03(20%) SC-04(15%) SC-05(10%)
  const score = Math.round(
    checks['sc01'].score * 0.30 +
    checks['sc02'].score * 0.25 +
    checks['sc03'].score * 0.20 +
    checks['sc04'].score * 0.15 +
    checks['sc05'].score * 0.10
  )
